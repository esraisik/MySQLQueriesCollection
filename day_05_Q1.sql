...
-- manav tablosundaki urunleri ve her bir urunun toplam satis miktarı(kg) listeleyin
SELECT urun_adi, SUM(miktar_kg) AS urunlerin_toplam_Ssatis_miktari_kg
FROM manav
GROUP BY urun_adi;
...
SELECT kategori,SUM(miktar_kg*satis_fiyati) AS urunlerinToplamGeliri
FROM manav
GROUP BY kategori;
...
FROM manav
GROUP BY urun_adi 
ORDER BY urunlerin_toplam_Ssatis_miktari_kg DESC
LIMIT 1 ;
...
SELECT satis_tarihi ,AVG(satis_fiyati) AS gunlukOrtalamaKgSatisFiyati
FROM manav
...
SELECT SUM(miktar_kg) AS toplam_meyve_satis
FROM manav
